...
const mapRowToOrder = (row: any, headers: string[]): Partial<Order> => {
  ...
  headers.forEach(originalHeaderFromFile => {
    ...
    let orderKey = headerMap[normalizedFileHeader];
    
    // If the normalized header maps to 'gp', but the original header contained '%',
    // it's likely a percentage column (e.g., "GP %") that got normalized to "gp".
    // Re-route it to 'gpPercentage'.
    if (orderKey === 'gp' && originalHeaderFromFile.includes('%')) {
      orderKey = 'gpPercentage';
    }
    ...
    if (orderKey) {
      ...
      if (orderKey === 'qty' || orderKey === 'unitPrice' || orderKey === 'exportValue' || orderKey === 'priceInUSD' || orderKey === 'importValue' || orderKey === 'gp') {
        ...
        let finalPercentageString = '0.00%'; // Default
        if (typeof value === 'number') {
          // If number is strictly between 0 and 1 (e.g. 0.25 for 25%), multiply by 100.
          // Otherwise (e.g. 25 for 25%, or 0 for 0%), use as is.
          if (value > 0 && value < 1) {
              finalPercentageString = `${(value * 100).toFixed(2)}%`;
          } else {
              finalPercentageString = `${value.toFixed(2)}%`;
          }
        } else if (typeof value === 'string' && value.trim() !== '') {
          const cleanedValue = value.replace(/%/g, '').replace(/,/g, '').trim();
          const numVal = parseFloat(cleanedValue);
          if (!isNaN(numVal)) {
            finalPercentageString = `${numVal.toFixed(2)}%`;
          } else if (value.trim().toLowerCase() === 'n/a' || value.trim().toLowerCase() === 'error') {
            // Preserve specific non-numeric strings
            finalPercentageString = value.trim();
          }
          // If string is unparseable and not N/A/Error, it defaults to '0.00%'
        }
        // If value is null, undefined, or an empty string not caught above, it defaults to '0.00%'
        (order[orderKey] as any) = finalPercentageString;
      } else {
