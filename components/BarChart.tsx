...
const BarChart: React.FC<BarChartProps> = ({
  ...
  const paddingTop = 40; 
  const paddingBottom = 90; // Increased for potentially 3 lines of X-axis labels
    ...
    <svg width="100%" height="100%" viewBox={`0 0 ${width} ${height}`} aria-label={title || "Bar chart"}>
      ...
      <g transform={`translate(${paddingLeft}, ${paddingTop})`}>
        ...
        {Array.from({ length: numYAxisTicks + 1 }).map((_, i) => {
          ...
          return (
            <g key={`y-tick-${i}`} className="text-slate-600">
              <line
                x1={-6} y1={yPos}
        ...
        {data.map((d, i) => {
          ...
          return (
            <g key={d.label} className="group">
              ...
              {barH > 12 && ( // Only show value label if bar is tall enough
                <text
                    x={x + barWidth / 2}
              ...
              <foreignObject x={x - barGroupWidth/2 + barWidth/2} y={chartHeight + 8} width={barGroupWidth} height={paddingBottom - 10}>
                 <p 
                    className="text-xs text-slate-700 text-center break-words leading-tight p-0.5" // Adjusted padding for better fit
                    style={{
                        display: '-webkit-box',
                        ...
                        maxHeight: '3.6em', // Approx 3 lines * 1.2em line-height
                    }}
                    title={d.label}
                  >
                   {d.label}
