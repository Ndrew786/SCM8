...
type MetricOptionKey = 'ExportValue' | 'ImportValue';
const METRIC_OPTIONS: { key: MetricOptionKey, label: string }[] = [
  { key: 'ExportValue', label: 'Export Value' },
  { key: 'ImportValue', label: 'Import Value' },
];

type GroupingOptionKey = 'ProductName' | 'ProductCode';
const GROUPING_OPTIONS: { key: GroupingOptionKey, label: string }[] = [
...
const DashboardPage: React.FC<DashboardPageProps> = ({ orders }) => {
  ...
  const [selectedMetric, setSelectedMetric] = useState<MetricOptionKey>('ExportValue');
  const [selectedGrouping, setSelectedGrouping] = useState<GroupingOptionKey>('ProductName');

  const dynamicSegmentOptions = useMemo(() => {
    if (!orders || orders.length === 0) {
      return [{ key: 'All', label: 'All Segments' }];
    }
    const uniqueSegments = Array.from(new Set(orders.map(order => order.segment).filter(s => typeof s === 'string' && s.trim() !== ''))).sort();
    const options = uniqueSegments.map(segment => ({ key: segment!, label: segment! }));
    return [{ key: 'All', label: 'All Segments' }, ...options];
  }, [orders]);

  const [selectedSegment, setSelectedSegment] = useState<string>(dynamicSegmentOptions[0].key);

  useEffect(() => {
    const currentSegmentStillValid = dynamicSegmentOptions.some(opt => opt.key === selectedSegment);
    if (!currentSegmentStillValid && dynamicSegmentOptions.length > 0) {
        setSelectedSegment(dynamicSegmentOptions[0].key); // Default to 'All' or first available
    } else if (dynamicSegmentOptions.length === 1 && dynamicSegmentOptions[0].key === 'All' && selectedSegment !== 'All') {
        setSelectedSegment('All'); // Only 'All' is available
    }
  }, [dynamicSegmentOptions, selectedSegment]);


  const displayData = useMemo((): DisplayEntry[] => {
    let filteredOrders = orders;
    if (selectedSegment !== 'All') {
      filteredOrders = orders.filter(order => order.segment === selectedSegment);
    }
  ...
  }, [orders, selectedSegment, selectedMetric, selectedGrouping, selectedTopN]);
  ...
  const dynamicTitle = useMemo(() => {
    ...
    const segmentLabelObj = dynamicSegmentOptions.find(opt => opt.key === selectedSegment);
  ...
  }, [selectedTopN, selectedSegment, selectedMetric, selectedGrouping, dynamicSegmentOptions]);
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-6">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                 {METRIC_OPTIONS.map(option => (
                  <ControlButton key={`metric-${option.key}`} value={option.key} selectedValue={selectedMetric} onClick={(value) => setSelectedMetric(value as MetricOptionKey)}>{option.label}</ControlButton>
                ))}
              </div>
            </div>
              ...
              <label htmlFor="grouping-select" className="block text-sm font-semibold text-slate-800 mb-1.5">Group By:</label>
               <div className="flex flex-wrap gap-2">
                {GROUPING_OPTIONS.map(option => (
                  <ControlButton key={`grouping-${option.key}`} value={option.key} selectedValue={selectedGrouping} onClick={(value) => setSelectedGrouping(value as GroupingOptionKey)}>{option.label}</ControlButton>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <select
                id="segment-select" value={selectedSegment} onChange={(e) => setSelectedSegment(e.target.value)}
              ...
              >
                {dynamicSegmentOptions.map(option => ( <option key={option.key} value={option.key}> {option.label} </option> ))}
              </select>
            </div>
          </div>
        </div>
